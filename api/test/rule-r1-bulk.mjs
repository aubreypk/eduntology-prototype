...
db.exec(`
  ...
  CREATE TABLE criterion (id TEXT PRIMARY KEY, code TEXT, process_id TEXT);
...
const CRITERIA = [
  ...
  ['C3', 'A.2.1', 'P03'],
  ['C4', 'A.3.1', 'P04'],
  ['C5', 'A.4.1', null]        // a criterion requiring no taught procedure
]
const ACTIVITIES = {
  ACT1: ['C1'],
  ACT2: ['C2'],
  ACT3: ['C2', 'C3'],          // two processes, met at different points
  ACT4: ['C5'],                // no process at all
  ACT5: ['C1', 'C2', 'C3', 'C4']
}
const LEARNERS = {
  L0: [],
...
for (const [id, code, process] of CRITERIA) {
  db.prepare('INSERT INTO criterion VALUES (?,?,?)').run(id, code, process)
}
